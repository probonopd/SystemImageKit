...
menuentry "$ISONAME - $LIVETOOL $LIVETOOLVERSION" --class fedora {
        ...
        live_args="for-dracut --> iso-scan/filename=\${iso_path} selinux=0 rd.live.deltadir=/run/initramfs/isoscan/boot/deltadir rd.live.user=$USERNAME rd.live.host=$HOSTNAME vconsole.keymap=$KEYBOARD locale.LANG=$LOCALE"
        custom_args=""
        iso_args="$APPEND"
